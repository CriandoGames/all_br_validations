...
Todos os métodos `validate*` retornam `Result<ValidationError, String>` em vez de `bool`. Em caso de sucesso, `Success` carrega o valor normalizado ou original conforme o contrato do método. Consulte a [documentação do pacote all_result](https://pub.dev/packages/all_result) para detalhes do tipo.
